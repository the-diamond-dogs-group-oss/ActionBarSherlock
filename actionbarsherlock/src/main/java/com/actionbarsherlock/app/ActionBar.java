 ...
 * <p>
 * Beginning with Android 3.0 (API level 11), the action bar appears at the top
 * of an activity's window when the activity uses the system's
 * {@link android.R.style#Theme_Holo Holo} theme (or one of its descendant
 * themes), which is the default. You may otherwise add the action bar by
 * calling {@link android.view.Window#requestFeature
 * requestFeature(FEATURE_ACTION_BAR)} or by declaring it in a custom theme with
 * the {@link android.R.styleable#Theme_windowActionBar windowActionBar}
 * property.
 * <p>
 * By default, the action bar shows the application icon on the left, followed
 * by the activity title. If your activity has an options menu, you can make
 * select items accessible directly from the action bar as "action items". You
 * can also modify various characteristics of the action bar or remove it
 * completely.
 * </p>
 * <p>
 * From your activity, you can retrieve an instance of {@link ActionBar} by
 * calling {@link android.app.Activity#getActionBar getActionBar()}.
 * </p>
 * <p>
 * In some cases, the action bar may be overlayed by another bar that enables
 * contextual actions, using an {@link android.view.ActionMode}. For example,
 * when the user selects one or more items in your activity, you can enable an
 * action mode that offers actions specific to the selected items, with a UI
 * that temporarily replaces the action bar. Although the UI may occupy the same
 * space, the {@link android.view.ActionMode} APIs are distinct and independent
 * from those for {@link ActionBar}. <div class="special reference">
 * <h3>Developer Guides</h3>
 * <p>
 * For information about how to use the action bar, including how to add action
 * items, navigation modes and more, read the <a href="{@docRoot}
 * guide/topics/ui/actionbar.html">Action Bar</a> developer guide.
 * </p>
 * </div>
 */
public abstract class ActionBar {
	 ...
	 * 
	 * <p>
	 * The display option {@link #DISPLAY_SHOW_CUSTOM} must be set for the
	...
	/**
	 * Set the icon to display in the 'home' section of the action bar. The
	 * action bar will use an icon specified by its style or the activity icon
	 * by default.
	 * 
	 * Whether the home section shows an icon or logo is controlled by the
	 * display option {@link #DISPLAY_USE_LOGO}.
	 * 
	 * @param resId
	...
	/**
	 * Set the logo to display in the 'home' section of the action bar. The
	 * action bar will use a logo specified by its style or the activity logo by
	 * default.
	 * 
	 * Whether the home section shows an icon or logo is controlled by the
	 * display option {@link #DISPLAY_USE_LOGO}.
	 ...
	 * 
	 * The supplied OnNavigationListener will alert the application when the
	 * user changes the current list selection.
	 * 
	 * @param adapter
	 *            An adapter that will provide views both to display the current
	 ...
	 * {@link #DISPLAY_SHOW_HOME} option. setDisplayOptions(DISPLAY_SHOW_HOME,
	 * DISPLAY_SHOW_HOME | DISPLAY_USE_LOGO) will enable
	 * {@link #DISPLAY_SHOW_HOME} and disable {@link #DISPLAY_USE_LOGO}.
	 * 
	 * @param options
	 *            A combination of the bits defined by the DISPLAY_ constants
	 *            defined in ActionBar.
	...
	/**
	 * Set whether to display the activity logo rather than the activity icon. A
	 * logo is often a wider, more detailed image.
	 * 
	 * <p>
	 * To set several display options at once, see the setDisplayOptions
	 * methods.
	 * 
	 * @param useLogo
	 ...
	 * 
	 * @param showCustom
	 *            true if the currently set custom view should be displayed,
	 *            false otherwise.
	 * 
	 * @see #setDisplayOptions(int)
	 * @see #setDisplayOptions(int, int)
	...
	/**
	 * Set the ActionBar's background. This will be used for the primary action
	 * bar.
	 * 
	 * @param d
	 *            Background drawable
	...
	/**
	 * Set the ActionBar's split background. This will appear in the split
	 * action bar containing menu-provided action buttons on some devices and
	 * configurations.
	 * <p>
	 ...
	 * 
	 * @param contentResId
	 *            Resource Id of the content description
	 */
	...
	/**
	 * Create and return a new {@link Tab}. This tab will not be included in the
	 * action bar until it is added.
	 * 
	 * <p>
	 * Very often tabs will be used to switch between {@link Fragment} objects.
	 * Here is a typical implementation of such tabs:
	 * </p>
	 * 
	 ...
	 * <code>position</code>. If this is the first tab to be added it will
	...
	/**
	 * Select the specified tab. If it is not a child of this action bar it will
	 * be added.
	 * 
	 * <p>
	 * Note: If you want to select by index, use
	 * {@link #setSelectedNavigationItem(int)}.
	 * </p>
	 * 
	 * @param tab
	 *            Tab to select
	 ...
	 * ActionBar does not have the feature
	 * {@link Window#FEATURE_ACTION_BAR_OVERLAY} it will resize application
	 * content to fit the new space available.
	 */
	...
	/**
	 * @return <code>true</code> if the ActionBar is showing, <code>false</code>
	 *         otherwise.
	 */
	 ...
	 * (Note that this is the application home/up affordance on the action bar,
	 * not the systemwide home button.)
	 * 
	 * <p>
	 * This defaults to true for packages targeting &lt; API 14. For packages
	 * targeting API 14 or greater, the application should call this method to
	 * enable interaction with the home/up affordance.
	 * 
	 * <p>
	 * Setting the {@link #DISPLAY_HOME_AS_UP} display option will automatically
	 * enable the home button.
	 * 
	 * @param enabled
	 *            true to enable the home button, false to disable the home
	 *            button.
	 */
	...
	/**
	 * Returns a {@link Context} with an appropriate theme for creating views
	 * that will appear in the action bar. If you are inflating or instantiating
	 * custom views that will appear in an action bar, you should use the
	 * Context returned by this method. (This includes adapters used for list
	 * navigation mode.) This will ensure that views contrast properly against
	 * the action bar.
	 * 
	...
	public interface OnMenuVisibilityListener {
		 ...
		 * 
		 * @param isVisible
		 *            True if an action bar menu is now visible, false if no
		 *            action bar menus are visible.
		 */
	...
	public static abstract class Tab {
		 ...
		 * 
		 * @param resId
		 *            A resource ID referring to the text that should be
		 *            displayed
		 * @return The current instance for call chaining
		 */
		...
		/**
		 * Set the {@link TabListener} that will handle switching to and from
		 * this tab. All tabs must have a TabListener set before being added to
		 * the ActionBar.
		 * 
		 * @param listener
		...
		/**
		 * Set a description of this tab's content for use in accessibility
		 * support. If no content description is provided the title will be
		 * used.
		 * 
		 * @param resId
		...
		/**
		 * Gets a brief description of this tab's content for use in
	...
	public interface TabListener {
		 ...
		 *            unselect and the newly selected tab's select will be
		 *            executed in a single transaction. This FragmentTransaction
		 ...
		 * @param ft
		 *            A {@link FragmentTransaction} for queuing fragment
		 *            operations to execute once this method returns. This
		 *            FragmentTransaction does not support being added to the
		 *            back stack.
		 */
	...
	public static class LayoutParams extends MarginLayoutParams {
		private static final int[] ATTRS = new int[] { android.R.attr.layout_gravity };
		 ...
		 */
		@ViewDebug.ExportedProperty(mapping = { @ViewDebug.IntToString(from = -1, to = "NONE"),

 ...
 * , read the <a href="{@docRoot}, read the <a href="{@docRoot}, read the <a
 * href="{@docRoot}, read the <a href="{@docRoot}
...
public class SearchView extends LinearLayout implements CollapsibleActionView {
	...
	View.OnKeyListener mTextKeyListener = new View.OnKeyListener() {
		...
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			if (DBG) {
				Log.d(LOG_TAG, "mTextListener.onKey(" + keyCode + "," + event + "), selection: " + mQueryTextView.getListSelection());
			}

			// If a suggestion is selected, handle enter, search key, and action
			// keys
			// as presses on the selected suggestion
			if (mQueryTextView.isPopupShowing() && mQueryTextView.getListSelection() != ListView.INVALID_POSITION) {
			...
			if (!mQueryTextView.isEmpty() && KeyEventCompat.hasNoModifiers(event)) {
				if (event.getAction() == KeyEvent.ACTION_UP) {
					if (keyCode == KeyEvent.KEYCODE_ENTER) {
						...
						launchQuerySearch(KeyEvent.KEYCODE_UNKNOWN, null, mQueryTextView.getText().toString());
						return true;
					}
				}
				if (event.getAction() == KeyEvent.ACTION_DOWN) {
					...
					// (actionKey.getQueryActionMsg() != null)) {
					// TODO launchQuerySearch(keyCode,
					// actionKey.getQueryActionMsg(), mQueryTextView
	...
	public void setQueryRefinementEnabled(boolean enable) {
		...
		if (mSuggestionsAdapter instanceof SuggestionsAdapter) {
			((SuggestionsAdapter) mSuggestionsAdapter).setQueryRefinement(enable ? SuggestionsAdapter.REFINE_ALL
	...
	private void updateSubmitArea() {
		int visibility = GONE;
		if (isSubmitAreaEnabled() && (mSubmitButton.getVisibility() == VISIBLE || mVoiceButton.getVisibility() == VISIBLE)) {
			visibility = VISIBLE;
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		// null)) {
		// TODO launchQuerySearch(keyCode, actionKey.getQueryActionMsg(),
		// mQueryTextView.getText()
		// TODO .toString());
		// TODO return true;
		// TODO }
	 ...
	 *         suggestion
	 */
	// TODO private static String getActionKeyMessage(Cursor c,
	...
	private void updateSearchAutoComplete() {
		...
		if (mSearchable.getSuggestAuthority() != null) {
			...
			mQueryTextView.setAdapter(mSuggestionsAdapter);
			((SuggestionsAdapter) mSuggestionsAdapter).setQueryRefinement(mQueryRefinement ? SuggestionsAdapter.REFINE_ALL
					: SuggestionsAdapter.REFINE_BY_ENTRY);
		}
	}
	 ...
	 *
	 * @param empty
	 *            whether the search query text field is empty. If it is, then
	...
	private void adjustDropDownSizeAndPosition() {
		if (mDropDownAnchor.getWidth() > 1) {
			...
			int iconOffset = mIconifiedByDefault ? res.getDimensionPixelSize(R.dimen.abs__dropdownitem_icon_width)
					+ res.getDimensionPixelSize(R.dimen.abs__dropdownitem_text_padding_left) : 0;
			mQueryTextView.getDropDownBackground().getPadding(dropDownPadding);
			mQueryTextView.setDropDownHorizontalOffset(-(dropDownPadding.left + iconOffset) + anchorPadding);
			mQueryTextView.setDropDownWidth(mDropDownAnchor.getWidth() + dropDownPadding.left + dropDownPadding.right + iconOffset
					- (anchorPadding));
		}
	}
	...
	public interface OnSuggestionListener {
		 ...
		 *
		 * @param position
		 *            the absolute position of the clicked item in the list of
		 *            suggestions.
		 *
		 * @return true if the listener handles the event and wants to override

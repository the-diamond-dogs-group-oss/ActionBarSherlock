...
public class ScrollingTabContainerView extends NineHorizontalScrollView implements IcsAdapterView.OnItemSelectedListener {
	...
	private IcsLinearLayout createTabLayout() {
		final IcsLinearLayout tabLayout = (IcsLinearLayout) LayoutInflater.from(getContext()).inflate(
				R.layout.abs__action_bar_tab_bar_view, null);
		tabLayout.setMeasureWithLargestChildEnabled(true);
		tabLayout.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
				LinearLayout.LayoutParams.MATCH_PARENT));
		return tabLayout;
	}

	private IcsSpinner createSpinner() {
		final IcsSpinner spinner = new IcsSpinner(getContext(), null, R.attr.actionDropDownStyle);
	...
	public void addTab(ActionBar.Tab tab, int position, boolean setSelected) {
		...
		mTabLayout.addView(tabView, position, new IcsLinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, 1));
		if (mTabSpinner != null) {
			((TabAdapter) mTabSpinner.getAdapter()).notifyDataSetChanged();
		}
		if (setSelected) {
